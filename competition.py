def zeros(lenx,leny):
	return [[0 for x in range(lenx)] for y in range(leny)]
def create_schedule(players):
	matchup=[]
	tv=['samsung','westinghouse']
	if len(players)==14:
	...
	competitionmatrix=zeros((len(players),len(players)))
	#cooperationmatirx=zeros((len(players),len(players)))
	for i in range(len(players)):
		for match in matchupcopy:
			for player in match['home']:
				for opponent in match['away']:
					competitionmatrix[player][opponent]=competitionmatrix[player][opponent]+1
					competitionmatrix[opponent][player]=competitionmatrix[opponent][player]+1
